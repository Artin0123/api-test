# API Key Tester — SPEC（2026-08-13 封存快照）

**此文件已停止维护，不再是权威规范。** 描述的是失效密钥功能加入之前的系统状态，已知落后于代码的部分：第 3 节缺 KV key `dead_keys`、第 7 节缺 4 个 `/api/dead-keys` 端点、第 9 节缺「失效密钥」分页。以代码为准。

描述架构、数据模型、API 契约、KV 设计、脚本行为、前端交互和部署流程。
