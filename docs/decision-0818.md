...
- 單列不再放刪除按鈕。改由面板標題列「删除密钥」（位於「重新读取」右側）進入勾選模式；按鈕切換為「取消删除」（勾選至少一筆後轉為「确认删除」，點擊後跳出既有 modal 二次確認）。表格第一欄平常顯示「编号」（由上到下顯示數字 1, 2, 3...），進入刪除模式時切換為紅框半透明勾選框（表頭為全選），第一欄設有固定欄寬（3.2rem）且其餘文字欄位限縮 max-width，避免切換時表格欄位跳動或產生橫向滾動條。
